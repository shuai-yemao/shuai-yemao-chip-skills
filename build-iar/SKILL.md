...
### 不该激活
- 工程中存在 `CMakeLists.txt` 但无 `.ewp`/`.eww` → 优先用 `build-cmake`
- 工程中存在 `.uvprojx`/`.uvproj` → 优先用 `build-keil`
- 工程中存在 `platformio.ini` → 优先用 `build-platformio`
- 用户在非 Windows 平台上 → IAR 仅 Windows 可用，应使用 `build-cmake` + ARM GCC
