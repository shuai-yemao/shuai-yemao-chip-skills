...
# ── 常量 ────────────────────────────────────────────────────────
AGENTPKG_NAME = "chip-embedded-3.1.0.agentpkg"
PLATFORM_NAMES = {
    "claude-code": "Claude Code CLI",
    "standalone":  "独立目录 (~/.chip-agent)",
}
...
def detect_platforms(exclude_vscode=True):
    """检测 Claude Code 目录，无则 fallback 到独立目录"""
    platforms = {}
    ...
    if os.path.isdir(cc_dir):
        ...
        # 无 Claude Code → 独立目录作为 fallback
        chip_skills = os.path.join(home, ".chip-agent", "skills")
        platforms["standalone"] = chip_skills
...
def install_skills_dir(tmpdir, skills_dir):
    """安装 skills 目录（先清理旧 skill，再安装新 skill），返回数量"""
    ...
    # 获取新包中的 skill 列表
    incoming = {d for d in os.listdir(src_skills)
                if os.path.isdir(os.path.join(src_skills, d))}

    # 清理旧 skill：删除目标目录中存在但新包中没有的
    removed = 0
    for existing in sorted(os.listdir(skills_dir)):
        existing_path = os.path.join(skills_dir, existing)
        if os.path.isdir(existing_path) and existing not in incoming:
            shutil.rmtree(existing_path, ignore_errors=True)
            removed += 1
    if removed > 0:
        print(f"      [i] 清理 {removed} 个旧 skill")
...
def install_to_standalone(tmpdir, manifest, base_dir):
    """独立目录 (~/.chip-agent/skills/)"""
    count = install_skills_dir(tmpdir, base_dir)
    localize_agent_config(tmpdir, os.path.dirname(base_dir))

    soul_src = os.path.join(tmpdir, "agent", "SOUL.md")
    if os.path.isfile(soul_src):
        shutil.copy2(soul_src, os.path.join(os.path.dirname(base_dir), "CLAUDE.md"))

    # 输出配置指引
    agents_dir = os.path.dirname(base_dir)
    print(f"\n      ┌{'─'*45}┐")
    print(f"      │ 独立安装完成                               │")
    print(f"      │ Skills: {agents_dir}/skills/          │")
    print(f"      │ CLAUDE.md: {agents_dir}/CLAUDE.md  │")
    print(f"      └{'─'*45}┘")
    print(f"      [!] 将 CLAUDE.md 复制到目标工具的工作目录")
    print(f"          或将 skills/ 注册到工具的 skills 路径")
    return count


INSTALLERS = {
    "claude-code": install_to_claude_code,
    "standalone":  install_to_standalone,
}
...
def print_report(manifest, results):
    ...
    hints = {
        "claude-code": "重启终端后生效",
        "standalone":  "将 CLAUDE.md 复制到工作目录即可使用",
    }
...
ENV_CHECKS = {
    "git":       {"check": "git --version",     "winget": "Git.Git",          "label": "Git"},
    "node":      {"check": "node --version",    "winget": "OpenJS.NodeJS",    "label": "Node.js"},
    "claude":    {"check": "claude --version",  "npm":    "@anthropic-ai/claude-code", "label": "Claude Code CLI"},
}

ENV_EXE_DIR = os.path.join(os.path.expanduser("~"), ".chip-agent", "bin")
...
def check_env():
    ...
    for key, cfg in ENV_CHECKS.items():
        rc = os.system(f"{cfg['check']} >nul 2>nul")
        if rc != 0:
            missing.append(key)
    return missing
...
def _find_npm_root():
    """查找 npm 全局安装目录"""
    try:
        result = subprocess.run("npm root -g", capture_output=True, text=True, shell=True, timeout=10)
        return result.stdout.strip() if result.returncode == 0 else None
    except Exception:
        return None


def _ensure_claude_config(tmpdir, claude_dir):
    """部署 Claude Code 配置：CLAUDE.md + settings.json + skills"""
    skills_dir = os.path.join(claude_dir, "skills")
    install_skills_dir(tmpdir, skills_dir)
    localize_agent_config(tmpdir, claude_dir)

    # CLAUDE.md
    soul_src = os.path.join(tmpdir, "agent", "SOUL.md")
    if os.path.isfile(soul_src):
        shutil.copy2(soul_src, os.path.join(claude_dir, "CLAUDE.md"))
    ...
    print(f"      [OK] Claude Code 配置完成: {len(os.listdir(skills_dir))} skills")
...
def install_missing(missing):
    ...
    for key in missing:
        ...
        if "winget" in cfg:
            rc = os.system(f"winget install --silent --accept-package-agreements {cfg['winget']} >nul 2>&1")
            if rc != 0:
                print(f"  [!] {cfg['label']} 安装失败（可手动安装）")
                continue
            print(f"  [OK] {cfg['label']} 安装完成")

        if "npm" in cfg:
            rc = os.system(f"npm install -g {cfg['npm']} >nul 2>&1")
            if rc != 0:
                print(f"  [!] {cfg['label']} 安装失败（npm 不可用）")
                continue
            print(f"  [OK] {cfg['label']} 安装完成")
            claude_installed = True

    # Claude Code 部署：找到 npm bin 目录并设置 PATH
    if claude_installed:
        npm_root = _find_npm_root()
        if npm_root:
            npm_bin = os.path.join(os.path.dirname(npm_root), "bin")
            if os.path.isdir(npm_bin):
                os.environ["PATH"] = npm_bin + os.pathsep + os.environ.get("PATH", "")
            # 验证
            rc = os.system("claude --version >nul 2>&1")
            if rc == 0:
                print(f"  [OK] claude 命令可用")
            else:
                print(f"  [!] 请重启终端后使用 claude 命令（PATH 已更新）")

    # 验证
    still_missing = check_env()
    if still_missing:
        print(f"\n  [!] 仍有 {len(still_missing)} 个组件未安装:")
        for k in still_missing:
            print(f"      - {ENV_CHECKS[k]['label']}")
        return False
    print(f"\n  [OK] 运行环境就绪")
    return True
...
def main():
    ...
    parser.add_argument("--setup-env", action="store_true",
                        help="自动安装缺失的运行时（Git/Node.js/Claude Code）")
    parser.add_argument("--setup-all", action="store_true",
                        help="安装运行环境 + Agent Skills 全自动安装")
    args = parser.parse_args()

    print_banner()

    # ── 环境自举 ──
    setup_mode = args.setup_env or args.setup_all
    if setup_mode:
        print("  [*] 检查运行环境...")
        missing = check_env()
        if missing:
            print(f"  [i] 需要安装: {', '.join(ENV_CHECKS[k]['label'] for k in missing)}")
            if args.yes:
                install_missing(missing)
            else:
                reply = input("  [?] 自动安装缺失组件? [Y/n] ").strip().lower()
                if reply in ("", "y", "yes"):
                    install_missing(missing)
                else:
                    print("  [-] 跳过环境安装")
        ...
        if args.setup_all and not check_env():
            print("  [X] 运行环境未就绪，跳过 Agent 安装")
            input("\n  按 Enter 退出...")
            return 1
    ...
    with tempfile.TemporaryDirectory(prefix="chip_install_") as tmpdir:
        ...
        # ── 如果 setup-all 且 Claude Code 目录存在，部署配置 ──
        if args.setup_all:
            claude_dir = os.path.join(os.path.expanduser("~"), ".claude")
            if os.path.isdir(claude_dir):
                print("  [*] 部署 Claude Code 配置...")
                _ensure_claude_config(tmpdir, claude_dir)
