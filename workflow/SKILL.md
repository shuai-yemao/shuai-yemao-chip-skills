---
name: workflow
description: 当需要串联多个 skill 完成嵌入式开发全流程时使用。支持 6 个专用 Agent 分工协作（编译/烧录/监控、开发循环、项目管理、硬件验证、发布管理、Bug 修复闭环），以及 24 条预置流水线覆盖从需求调研到量产发布的全生命周期。用户提到流水线、一键编译烧录、全流程、多 Agent、分工协作时使用。
version: "3.1.0"
---
...
flowchart TB
    ...
    subgraph Skills["底层 Skill"]
        ...
        META["brainstorming\nkb-search\nwriting-plans"]
        CMB["cmbacktrace-debug\n故障诊断"]
        ELOG["elog-module\n日志管理"]
        RTT["segger-rtt-module\nRTT 传输"]
        LNK["linker-scatter\n链接脚本"]
        INT["interrupt-optimization\n中断优化"]
        LEARN["embedded-learning-notes\n笔记沉淀"]
    end
