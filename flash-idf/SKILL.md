...
### 不该激活
- 用户的目标芯片是 **STM32/nRF/GD32** 等非 ESP 系列 → 优先用 `flash-openocd` / `flash-jlink`
- 用户使用的是 Arduino IDE 而非 ESP-IDF → 使用 `flash-platformio`
- 用户的 ESP32 工程基于 PlatformIO 构建 → 使用 `flash-platformio`
- 用户只需要查看串口日志而非烧录 → 使用 `serial-monitor`
