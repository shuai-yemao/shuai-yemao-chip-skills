...
### 不该激活
- 工程中存在 `CMakeLists.txt` 但无 `platformio.ini`（CMake 工程）→ 优先用 `build-cmake`
- 工程中存在 `.uvprojx`/`.uvproj`（Keil）→ 优先用 `build-keil`
- 工程中存在 `.ewp`/`.eww`（IAR）→ 优先用 `build-iar`
- 工程是 ESP-IDF 结构而非 Arduino-ESP32 框架 → 优先用 `build-idf`
- 用户只需要上传（烧录）而不需要编译 → 使用 `flash-platformio`
