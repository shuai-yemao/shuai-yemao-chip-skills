...
### 不该激活
- 用户的目标芯片是 **ESP32/ESP8266** 系列 → 优先用 `flash-idf` / `flash-platformio`
- 用户已有 Keil MDK 且工程已配置调试器 → 优先用 `flash-keil`（使用工程内 Flash 算法）
- 用户明确说 "J-Link 烧录" → 优先用 `flash-jlink`（JLinkExe 生态）
