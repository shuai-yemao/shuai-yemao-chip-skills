...
| `cmbacktrace-debug` | CmBacktrace 故障自动追踪 (HardFault/断言) |
| `embedded-learning-path-framework` | 三阶段学习路径规划 |
| `embedded-learning-notes` | 学习笔记管理 (Obsidian 归档) |
...
| `sfud-module` | SFUD 串行 Flash 通用驱动 (W25Q64等) |
| `segger-rtt-module` | SEGGER RTT 实时传输/调试日志 |
| `elog-module` | EasyLogger 分级日志库 (RTT/UART) |
| `cmbacktrace-debug` | CmBacktrace 故障自动追踪 (HardFault) |
...
| HardFault 分析 | `cmbacktrace-debug` (一键诊断) → `arm-core-registers` (CFSR/HFSR 解码) |
