...
| DHT11 | 1-Wire (单总线) | 温湿度 | 直接实现 | 中 |
| DHT22 (AM2302) | 1-Wire (单总线) | 温湿度 | 直接实现 | 中 |
...
## DHT11 / DHT22 (AM2302) — 单总线温湿度传感器

**推荐方案**：无需开源库，直接实现单总线协议（GPIO 开漏 + 外部上拉 4.7kΩ）

**适配难度**：中

**适配要点**：
- 单总线协议：主机拉低 ≥ 18ms 发送复位信号 → 传感器拉低 80us 应答 → 拉高 80us 准备
- 40 位数据：DHT11 = 湿度整数+小数 + 温度整数+小数 + 校验和
- DHT22 = 湿度(H<<8|L)/10 + 温度(H<<8|L)/10 + 校验和，bit15 为负温标志
- 数据位编码："0" = 50us 低 + 26-28us 高，"1" = 50us 低 + 70us 高
- 采样点：DHT11 建议起始后 40us，DHT22 建议 30us
- 微秒延时：使用 DWT 周期计数器（Cortex-M）或硬件定时器，禁止使用软件循环延时

**常见陷阱**：
- **致命**：用 DHT11 解析代码读 DHT22 → 数据格式完全不同（8-bit 整数+小数 vs 16-bit/10）
- STM32F1 上 `HAL_GPIO_PULLUP` 不生效，需直接写 CRL 寄存器 + BSRR 置位 ODR
- 内部 ~40kΩ 上拉不足以驱动 DATA 线，推荐外部 4.7kΩ 上拉到 VCC
- FreeRTOS 环境下必须用 `taskENTER_CRITICAL()` 保护 40 位连续读取
- 读取间隔 ≥ 1 秒（DHT22 最高采样率 1Hz）
- 软件循环延时被编译器优化掉 → 必须用硬件计数器（DWT 或 TIM）

**关键代码片段**：
```c
// DHT22 数据解析 (与 DHT11 完全不同!)
uint16_t humi_raw = ((uint16_t)buf[0] << 8) | buf[1];
uint16_t temp_raw = ((uint16_t)buf[2] << 8) | buf[3];
*humi = (float)humi_raw / 10.0f;
if (temp_raw & 0x8000)
    *temp = -(float)(temp_raw & 0x7FFF) / 10.0f;  // 负温
else
