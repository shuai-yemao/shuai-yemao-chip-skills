...
- Linux、macOS、Windows 的串口命名规则复用 [platform-compatibility.md]({USER_HOME}/work/open-git/em_skill/shared/platform-compatibility.md)。
...
### 不该激活
- 用户使用的是 SEGGER RTT 而非 UART 日志 → 使用 `rtt-monitor`
- 用户需要的是 CAN 总线数据而非串口日志 → 使用 `can-debug`
- 用户需要的是 Modbus RTU 协议调试 → 使用 `modbus-debug`
- 用户需要的是 VISA 仪器通信（SCPI）→ 使用 `visa-debug`
